...
class Vocabulary(object):
    def __init__(self, vocab_file, emb_file='', dim_emb=0):
        with open(vocab_file, 'rb') as f:
            self.size, self.word2id, self.id2word = pickle.load(f) # сопоставление слов с их индексами и сопоставление индексов с их словами
        self.dim_emb = dim_emb
        self.embedding = np.random.random_sample(
            (self.size, self.dim_emb)) - 0.5 # инициализация матрицы эмбеддингов случайными значениями в диапазоне от -0.5 до 0.5


        # если указан файл с векторными представлениями (emb_file), то загружает векторы и обновляет эмбеддинги для слов, которые есть в словаре
        if emb_file:
