...
# состояние в процессе декодировани
class BeamState(object):
    def __init__(self, h, inp, sent, nll):
        self.h, self.inp, self.sent, self.nll = h, inp, sent, nll # nll - отрицательная логарифмическая вероятность (negative log likelihood) для текущего состояния
...
class Decoder(object):
    ...
    def __init__(self, sess, args, vocab, model):
        ...
        self.sess = sess # sess - сессия TensorFlow
        ...
        # определение входных данных и скрытых состояния как тензоров
        # cоздание эмбеддингов и проекции для выхода
    ...
    def decode(self, h):
        ...
        # декодирование, обновляя состояния и генерируя новые слова на каждом шаге
        for t in range(self.max_len):
            exp = [[] for i in range(batch_size)]
            for state in beam:
                # для получения логарифмических вероятностей и обновления состояний
            ...
            # выбор лучших  состояний для следующего шага
    ...
    # преобразование индексов в слова с помощью словаря, удаление спец. символов конца последовательности
