...
# удаление специального токена конца последовательности
def strip_eos(sents):
    return [sent[:sent.index('<eos>')] if '<eos>' in sent else sent
        for sent in sents]


# создание словаря для передачи модели
...
# создание нового списока, повторяя элементы из _x, чтобы получить длину n
def makeup(_x, n):
    x = []
    for i in range(n):
...
# переставление элементов списка _x
...
# добавление шума в x, заменяя некоторые слова на токен неизвестного слова (<unk>) и сортировка
...
# cоздание батча для обучения, включая входные и целевые последовательности
